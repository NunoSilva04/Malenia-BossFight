...
- **Parameters:**  `const n_vector& other`  
- **Return:**  `None`   
...
- **Return:**  `bool`
- **Description:** Pops the first element of the vector, returning true, and creates a new vector with one less element and copies the content of the old vector into the new vector. Populates the `popped_value` with the element popped from the vector If the vector is empty it will simply return false and `popped_value` will be have garbage value.
